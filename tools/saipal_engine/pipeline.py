...
def _candidates_for_session(
    ...
    for episode in episodes:
        refined = analyze_episode(
            episode, bundle, record, None, registry=registry
        )
...
def analyze_sessions(
    ...
    *,
    ...
    for record in index.get("sessions", []):
        ...
        for episode in episodes:
            if events_analyzed >= int(budget.max_events) or time.monotonic() - started >= float(getattr(budget, "time_limit_seconds", 3600)):
                budget_hit = True
                break
            candidates = _candidates_for_session(
                record, bundle, [episode], data, historical
            )
            if not candidates:
            ...
            for finding in advanced:
                ...
                if not findings_mod.qualification_threshold(
                    ...
                    continue
                emit_audit(home, finding, record, bundle, paths, data, audits_emitted)
            analyzed_up_to = max(analyzed_up_to, int(episode.get("end_seq", 0)))
            events_analyzed += _span_size(episode, bundle)
            if findings_created >= int(budget.max_candidates) or len(audits_emitted) >= int(getattr(budget, "max_audits", 10)):
                budget_hit = True
                break
            telemetry = hardening_mod.increment(
                telemetry, "events_analyzed", _span_size(episode, bundle)
            )

        sessions_analyzed += 1
        telemetry = hardening_mod.increment(telemetry, "sessions_analyzed")
        record["analysis"] = {
            "episodes_exhausted": not budget_hit,
            "last_analyzed_at": None,
            "analyzed_up_to_seq": analyzed_up_to,
            "next_episode_index": len([e for e in episodes if int(e.get("end_seq", 0)) <= analyzed_up_to]),
        }
        lease.release()
        ...
        _record_recurrence(home, record, session_findings, registry=data)
    ...
    try:
        ...
        for finding in session_findings:
            recurrence_mod.record_occurrence(
                home, finding, record, registry=registry
            )
