...
def _read_lines(file_path: str) -> list[str]:
    return [line for line in Path(file_path).read_text(encoding="utf-8").splitlines() if line.strip()]
...
def read_evidence(
    ...
    *,
    ...
    """One bounded read-only evidence window around a Claude event."""
